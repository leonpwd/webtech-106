...
export default function ChampionCard({ champion }: { champion: any }) {
  const imgFile =
    (champion.image && (champion.image.full || champion.image)) || "";
  const imageUrl = imgFile
    ? championImageUrl(imgFile, champion.version || undefined)
  ...
  return (
    <Link
      ...
      className="block p-4 border rounded-md hover:shadow-lg bg-card/60 border-border transition"
    >
