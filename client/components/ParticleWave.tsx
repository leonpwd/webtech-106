...
export default function ParticleWave({ color = '#ffffff', density = 14, amplitude = 12 }: Props) {
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      colOffsets = new Array(cols).fill(0).map(() => Math.random() * Math.PI * 2);
      colAmp = new Array(cols).fill(0).map(() => 0.6 + Math.random() * 0.8);
      cellJitter = Array.from({ length: cols }, () => Array.from({ length: rows }, () => (Math.random() - 0.5) * density * 0.4));
    };
    ...
    let colOffsets = new Array(cols).fill(0).map(() => Math.random() * Math.PI * 2);
    let colAmp = new Array(cols).fill(0).map(() => 0.6 + Math.random() * 0.8); // per-column amplitude multiplier
    let cellJitter: number[][] = Array.from({ length: cols }, () => Array.from({ length: rows }, () => (Math.random() - 0.5) * density * 0.4));
    ...
    const draw = (t: number) => {
      ...
      for (let col = 0; col < cols; col++) {
        const x = col * density + density / 2 + (Math.sin(time * 0.3 + col) * density * 0.08);
        ...
        for (let row = 0; row < rows; row++) {
          ...
          const layer2 = Math.sin(time * 0.6 + col * 0.6 + row * 0.05 + phase * 0.7);
          const layer3 = Math.sin(time * 1.9 + col * 0.12 - row * 0.09 + phase * 1.3);
          const noise = (layer1 * 0.6 + layer2 * 0.3 + layer3 * 0.1);
          const offset = noise * amplitude * ampMult * Math.cos(row * 0.06 + time * 0.5);
          const jitter = (cellJitter[col] && cellJitter[col][row]) || 0;
          const y = yBase + offset + jitter;
          // pixel size variation for depth
          const pixelSize = basePixelSize + Math.abs(Math.sin(time * 0.9 + col * 0.1 + row * 0.05)) * 0.9;
          // small alpha variation for softness
          const alpha = 0.28 + 0.7 * Math.abs(Math.sin(time * 0.8 + col * 0.2 + row * 0.05));
