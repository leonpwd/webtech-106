  ...
  async function handleDiscordOAuth() {
    ...
    try {
      ...
      if (redirectTo2 && !/^[a-zA-Z][a-zA-Z\d+\-.]*:\/\//.test(redirectTo2)) {
        if (!/^localhost(:|$)/.test(redirectTo2))
          redirectTo2 = `https://${redirectTo2}`;
      }
      if (
        process.env.NODE_ENV === "development" &&
        typeof window !== "undefined"
      )
        console.log("OAuth redirectTo:", redirectTo2);
      const { error: oauthError } = await supabase.auth.signInWithOAuth({
        provider: "discord",
        options: { redirectTo: redirectTo2 },
      });
      if (oauthError) setError(oauthError.message);
    } catch (err: any) {
      setError(err?.message || String(err));
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex items-center justify-between">
              ...
              <Link
                href="/auth/register"
                className="text-sm text-primary underline ml-3"
              >
                  ...
                  {loading ? (
                    ...
                    <>
                      ...
                      >
                        ...
                        >
                          <path
                            d="M60.104 4.552A58.3 58.3 0 0046.6.9a41.7 41.7 0 00-1.98 4.086 55.3 55.3 0 00-14.28 0A41.6 41.6 0 0028.4.9c-5.5 1.05-10.5 2.8-15.5 4.5C4.6 18.6 2.1 32.9 3.5 47.1a60.7 60.7 0 0018.1 5.3c1.4-2 2.7-4 3.9-6.1-5.7-1.7-10-4.8-13-8.4 1-.7 2-1.5 2.9-2.3 2.2 1.6 5 3 9 3.9 5.2.9 10.4.8 15.6 0 4-.8 6.8-2.2 9-3.9.9.8 1.9 1.6 2.9 2.3-2.9 3.6-7.3 6.7-13 8.4 1.3 2.1 2.6 4.1 3.9 6.1a60.6 60.6 0 0018.1-5.3c1.6-15.4-1-29.7-8.4-42.5z"
                            fill="currentColor"
                          />
                        </svg>
                      </span>
